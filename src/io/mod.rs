use crate::{col::HashMap, common::{CommodityIdx, EdgeIdx, NodeIdx}, demand::Demand, graph::Graph};
...
pub fn read_graph(path: &std::path::PathBuf) -> ExternalGraph {
    ...
    } else if path.extension().is_some_and(|it| it == "sqlite" || it == "sqlite3") {
...
pub fn read_demand(path: &std::path::PathBuf, ext_graph: &ExternalGraph) -> (Demand, Option<HashMap<i64, CommodityIdx>>) {
    if path.extension().is_some_and(|it| it == "tntp") {
